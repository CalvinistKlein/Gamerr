...
def create_unique_game_slug(text: str, existing_id: int = None) -> str:
    """Create a URL-friendly unique slug for a game, preventing duplicate slug collisions."""
    base_slug = create_slug(text) or 'game'
    slug = base_slug
    try:
        from flask import has_app_context
        if has_app_context():
            counter = 1
            query = Game.query.filter_by(slug=slug)
            if existing_id:
                query = query.filter(Game.id != existing_id)
...
class Game(db.Model):
    ...
    def __init__(self, **kwargs):
        platform_val = kwargs.pop('platform', None)
        region_val = kwargs.pop('region', None)
        file_path_val = kwargs.pop('file_path', None)
        file_size_val = kwargs.pop('file_size', None)
        super(Game, self).__init__(**kwargs)
        if not self.slug and self.title:
            self.slug = create_unique_game_slug(self.title, getattr(self, 'id', None))
        elif self.slug:
            self.slug = create_unique_game_slug(self.slug, getattr(self, 'id', None))
        if platform_val:
            self.platform = platform_val
        if region_val or file_path_val or file_size_val:
            self._ensure_rom_file(file_path=file_path_val, file_size=file_size_val, region=region_val)

    def _ensure_rom_file(self, file_path=None, file_size=None, region=None):
        import os
        from pathlib import Path
        path_str = file_path or (f"/data/roms/{self.slug}.rom" if self.slug else "unknown.rom")
        name_str = os.path.basename(path_str) if path_str else "unknown.rom"
        ext_str = Path(path_str).suffix or ".rom"
        
        if self.rom_files:
            rf = self.rom_files[0]
            if file_path is not None:
                rf.file_path = file_path
                rf.file_name = os.path.basename(file_path) if file_path else name_str
                rf.file_extension = Path(file_path).suffix or ext_str
            if file_size is not None:
                rf.file_size = file_size
    ...
    def region(self, value):
        if value:
            self._ensure_rom_file(region=value)
    ...
    def file_path(self, value):
        if value is not None:
            self._ensure_rom_file(file_path=value)
    ...
    def file_size(self, value):
        if value is not None:
            self._ensure_rom_file(file_size=value)
    ...
    def get_library_count(cls):
        ...
        return cls.query.filter(cls.status.in_(['library', 'active'])).count()
...
class ROMFile(db.Model):
    ...
    file_extension = db.Column(db.String(10), default='.rom', nullable=True)
...
def create_game_from_rom_file(rom_file_data: dict) -> Game:
    ...
    game = Game(
        ...
        slug=create_unique_game_slug(title) if title else 'unknown-game',
        status='active'
    )
...
def initialize_default_data():
    ...
    default_platforms = [
        ...
        {'name': 'PlayStation 3', 'slug': 'ps3', 'manufacturer': 'Sony', 'generation': 7},
        {'name': 'PSP', 'slug': 'psp', 'manufacturer': 'Sony', 'generation': 7},
        {'name': 'Xbox', 'slug': 'xbox', 'manufacturer': 'Microsoft', 'generation': 6},
        {'name': 'Xbox 360', 'slug': 'xbox-360', 'manufacturer': 'Microsoft', 'generation': 7},
        {'name': 'Dreamcast', 'slug': 'dreamcast', 'manufacturer': 'Sega', 'generation': 6},
        {'name': 'Nintendo DS', 'slug': 'ds', 'manufacturer': 'Nintendo', 'generation': 7},
        {'name': 'Nintendo Switch', 'slug': 'switch', 'manufacturer': 'Nintendo', 'generation': 8},
        {'name': 'Arcade', 'slug': 'arcade', 'manufacturer': 'Various', 'generation': 1},
        # PC and Microcomputers (PC games map dynamically to generations by release era)
        {'name': 'PC (DOS)', 'slug': 'dos', 'manufacturer': 'IBM / Microsoft', 'generation': None},
        {'name': 'PC (Windows)', 'slug': 'windows', 'manufacturer': 'Microsoft', 'generation': None},
        {'name': 'ScummVM', 'slug': 'scummvm', 'manufacturer': 'Various', 'generation': None},
        {'name': 'Commodore 64', 'slug': 'c64', 'manufacturer': 'Commodore', 'generation': 3},
        {'name': 'Commodore Amiga', 'slug': 'amiga', 'manufacturer': 'Commodore', 'generation': 4},
    ]
    
    for platform_data in default_platforms:
        p = Platform.query.filter_by(slug=platform_data['slug']).first()
        if not p:
            p = Platform(**platform_data)
            db.session.add(p)
        else:
            if p.generation is None and platform_data.get('generation') is not None:
                p.generation = platform_data['generation']
            if not p.manufacturer and platform_data.get('manufacturer'):
                p.manufacturer = platform_data['manufacturer']
