...
class IGDBClient:
    ...
    def _make_request(self, endpoint: str, query: str, retries: int = 3):
        """
        ...
        Args:
            ...
            retries: remaining retry attempts for rate limiting
        ...
        try:
            ...
            if response.status_code == 429:
                if retries <= 0:
                    logger.error("IGDB rate limit exceeded after max retries")
                    return None
                retry_after = int(response.headers.get('Retry-After', 60))
                logger.warning(f"Rate limited. Waiting {retry_after} seconds... ({retries} retries left)")
                time.sleep(retry_after)
                return self._make_request(endpoint, query, retries=retries - 1)
    ...
    def search_games(self, query: str, platform_id: Optional[int] = None, limit: int = 10) -> List[IGDBGame]:
        ...
        search_query = f'search "{safe_query}";'
