...
class LibretroImporter:
    ...
    def _parse_game_comment(cls, comment: str) -> Tuple[str, Optional[str]]:
        ...
        KNOWN_REGIONS = {'USA', 'EUR', 'JPN', 'Japan', 'Europe', 'World', 'Korea', 'Asia', 'Brazil', 'Australia', 'France', 'Germany', 'Italy', 'Spain', 'China', 'Netherlands', 'Sweden', 'Canada', 'UK'}
        all_parens = re.findall(r'\(([^)]+)\)', comment)
        region_match = None
        for group in all_parens:
            if group in KNOWN_REGIONS:
                region_match = group
                break
        
        if region_match:
            region_text = region_match
    ...
    def initialize_catalog(cls, dat_path: str = None) -> Dict:
        """Initialize the game catalog with libretro database or fallback seed data"""
        if not dat_path:
        ...
        if os.path.exists(dat_path):
            ...
            return cls.import_dat_directory(dat_path, limit_per_platform=50)
        
        # Fallback to initial_catalog.json seed file
        seed_path = os.path.join(os.path.dirname(__file__), '..', 'seeds', 'initial_catalog.json')
        if os.path.exists(seed_path):
            print(f"Initializing catalog from seed file: {seed_path}")
            try:
                ...
                for item in seed_games:
                    ...
                    if not existing:
                        gc = GameCatalog(
                            ...
                            source='seed',
                            source_id=f"{item.get('platform')}_{item.get('title')}",
                            last_updated=datetime.utcnow()
                        )
                        db.session.add(gc)
                        imported += 1
                db.session.commit()
                print(f"Seed catalog initialized: {imported} games imported")
                return {'imported_games': imported, 'total_games': len(seed_games)}
            except Exception as e:
                print(f"Error loading seed catalog: {e}")
                db.session.rollback()
                return {'error': str(e)}

        return {'error': f'DAT path not found: {dat_path} and no seed file available'}
