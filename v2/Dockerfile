...
RUN mkdir -p /data/roms /app/instance \
    && chmod -R 755 /data /app/instance
...
# Copy only necessary application files (exclude large data directories)
...
# Default command — starts gunicorn WSGI server
