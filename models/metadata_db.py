...
game_genre_association = db.Table('metadata_game_genre_association',
    db.Column('game_id', db.Integer, ForeignKey('metadata_games.id', ondelete='CASCADE'), primary_key=True),
    db.Column('genre_id', db.Integer, ForeignKey('metadata_genres.id', ondelete='CASCADE'), primary_key=True),
    db.Column('created_at', db.DateTime, default=datetime.utcnow)
)

game_theme_association = db.Table('game_theme_association',
    db.Column('game_id', db.Integer, ForeignKey('metadata_games.id', ondelete='CASCADE'), primary_key=True),
    db.Column('theme_id', db.Integer, ForeignKey('metadata_themes.id', ondelete='CASCADE'), primary_key=True),
    db.Column('created_at', db.DateTime, default=datetime.utcnow)
)

game_game_mode_association = db.Table('game_game_mode_association',
    db.Column('game_id', db.Integer, ForeignKey('metadata_games.id', ondelete='CASCADE'), primary_key=True),
    db.Column('game_mode_id', db.Integer, ForeignKey('metadata_game_modes.id', ondelete='CASCADE'), primary_key=True),
    db.Column('created_at', db.DateTime, default=datetime.utcnow)
)

game_player_perspective_association = db.Table('game_player_perspective_association',
    db.Column('game_id', db.Integer, ForeignKey('metadata_games.id', ondelete='CASCADE'), primary_key=True),
    db.Column('player_perspective_id', db.Integer, ForeignKey('metadata_player_perspectives.id', ondelete='CASCADE'), primary_key=True),
    db.Column('created_at', db.DateTime, default=datetime.utcnow)
)

game_keyword_association = db.Table('game_keyword_association',
    db.Column('game_id', db.Integer, ForeignKey('metadata_games.id', ondelete='CASCADE'), primary_key=True),
    db.Column('keyword_id', db.Integer, ForeignKey('metadata_keywords.id', ondelete='CASCADE'), primary_key=True),
    db.Column('created_at', db.DateTime, default=datetime.utcnow)
)

game_alternative_name_association = db.Table('game_alternative_name_association',
    db.Column('game_id', db.Integer, ForeignKey('metadata_games.id', ondelete='CASCADE'), primary_key=True),
    db.Column('alternative_name_id', db.Integer, ForeignKey('metadata_alternative_names.id', ondelete='CASCADE'), primary_key=True),
    db.Column('created_at', db.DateTime, default=datetime.utcnow)
)

game_developer_association = db.Table('metadata_game_developer_association',
    db.Column('game_id', db.Integer, ForeignKey('metadata_games.id', ondelete='CASCADE'), primary_key=True),
    db.Column('company_id', db.Integer, ForeignKey('metadata_companies.id', ondelete='CASCADE'), primary_key=True),
    db.Column('role', db.String(20), default='developer'),  # developer, publisher, etc.
    db.Column('created_at', db.DateTime, default=datetime.utcnow)
)

game_publisher_association = db.Table('metadata_game_publisher_association',
    db.Column('game_id', db.Integer, ForeignKey('metadata_games.id', ondelete='CASCADE'), primary_key=True),
    db.Column('company_id', db.Integer, ForeignKey('metadata_companies.id', ondelete='CASCADE'), primary_key=True),
...
game_platform_association = db.Table('metadata_game_platform_association',
    db.Column('game_id', db.Integer, ForeignKey('metadata_games.id', ondelete='CASCADE'), primary_key=True),
    db.Column('platform_id', db.Integer, ForeignKey('metadata_platforms.id', ondelete='CASCADE'), primary_key=True),
    db.Column('created_at', db.DateTime, default=datetime.utcnow)
)
    ...
    def to_dict(self, include_relationships=False):
        ...
        if include_relationships:
            ...
            if self.artwork_urls:
                try:
                    data['artwork_urls'] = json.loads(self.artwork_urls)
                except:
                    data['artwork_urls'] = []
            else:
                data['artwork_urls'] = []
            ...
            if self.video_urls:
                try:
                    data['video_urls'] = json.loads(self.video_urls)
                except:
                    data['video_urls'] = []
            else:
                data['video_urls'] = []
...
class MetadataDatabaseManager:
    ...
    def clear_database(cls):
        ...
        try:
            # Clear association tables first to avoid FK constraint violations
            db.session.execute(game_genre_association.delete())
            db.session.execute(game_theme_association.delete())
            db.session.execute(game_game_mode_association.delete())
            db.session.execute(game_player_perspective_association.delete())
            db.session.execute(game_keyword_association.delete())
            db.session.execute(game_alternative_name_association.delete())
            db.session.execute(game_developer_association.delete())
            db.session.execute(game_publisher_association.delete())
            db.session.execute(game_platform_association.delete())
