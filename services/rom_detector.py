...
class ROMDetector:
    ...
    # High confidence unambiguous file extensions
    EXTENSION_MAP = {
        # Nintendo
        '.nes': 'NES',
        '.smc': 'SNES',
    ...
    PLATFORM_PATTERNS = {
        ...
        'PlayStation 4': [r'\(ps4\)', r'\[ps4\]', r'\bplaystation ?4\b', r'\bps4\b', r'spider-man'],
        'PlayStation 3': [r'\(ps3\)', r'\[ps3\]', r'\bplaystation ?3\b', r'\bps3\b', r'uncharted 3'],
        'PlayStation 2': [r'\(ps2\)', r'\[ps2\]', r'\bplaystation ?2\b', r'\bps2\b', r'god of war'],
        'PlayStation': [r'\(ps1\)', r'\(psx\)', r'\[ps1\]', r'\[psx\]', r'\bplaystation\b', r'\bps1\b', r'\bpsx\b', r'final fantasy vii'],
        'Xbox One': [r'\bxbox ?one\b'],
        'Xbox 360': [r'\.xex$', r'\(xbox ?360\)', r'\bxbox ?360\b', r'gears of war'],
        'Xbox': [r'\.xbe$', r'\(xbox\)', r'\bxbox\b', r'halo'],
        'Arcade': [r'\(arcade\)', r'\(mame\)', r'\barcade\b', r'street fighter'],
        'PC (DOS)': [r'\.dosbox$', r'\(dos\)', r'\[dos\]', r'\bms-?dos\b'],
        'PC (Windows)': [r'\(windows\)', r'\[windows\]', r'\(pc\)', r'\[pc\]', r'\bwin95\b', r'\bwin98\b'],
        'ScummVM': [r'\.svm$', r'\(scummvm\)', r'\[scummvm\]', r'\bscummvm\b'],
        'Commodore 64': [r'\.d64$', r'\.t64$', r'\(c64\)', r'\[c64\]', r'\bc64\b', r'\bcommodore ?64\b'],
        'Commodore Amiga': [r'\.adf$', r'\.ipf$', r'\(amiga\)', r'\[amiga\]', r'\bamiga\b'],
    }
    ...
    def detect_platform_from_filename(cls, filename: str) -> Optional[str]:
        ...
        # 1. Unambiguous extension match
        if file_ext in cls.EXTENSION_MAP:
            return cls.EXTENSION_MAP[file_ext]
        
        # 2. Check for explicit platform patterns in filename
        for platform, patterns in cls.PLATFORM_PATTERNS.items():
            for pattern in patterns:
                if re.search(pattern, filename_lower, re.IGNORECASE):
                    return platform
        
        # 3. Fallback resolution for multi-platform container extensions
        if file_ext == '.pkg':
    ...
    def extract_title_from_filename(cls, filename: str) -> str:
        ...
        patterns_to_remove = [
            ...
            # File identifiers
            r'\.(?:nes|smc|sfc|n64|z64|v64|gb|gbc|gba|nds|3ds|cia|gen|md|smd|iso|bin|cue|chd|pbp|gcm|wbfs|nsp|xci|xbe|xex|pkg|zip|7z|rar)$',  # Known ROM/archive extensions only
        ]
        ...
        # Clean up: remove extra spaces, underscores, dots, hyphens
